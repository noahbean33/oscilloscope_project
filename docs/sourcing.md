...
All **63 generic passives** in Oscilloscope Rev 0.2 now have an exact production manufacturer part number: **33 resistors** and **30 capacitors**. Existing values, connectivity, and footprints were preserved. The four optional trim capacitors remain DNI and are excluded from both BOM and pick-and-place outputs.
...
- C16, C17, C23, and C24 retain `Exclude from BOM: true` and now also have `Exclude from Pick & Place: true`, while retaining documented optional-population MPNs.
...
BOM summary shows **33 resistor instances** and **26 populated capacitor instances**. The difference from 30 schematic capacitors is the four intentionally excluded DNI trim capacitors. No package changes were required, and every selected MPN package agrees with its existing 0603, 0805, 1206, or 2010 footprint.
