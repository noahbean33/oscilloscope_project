...
This file records the implemented **Oscilloscope internal-ADC MVP** decisions as of 2026-08-28. The live design has 101 components and 57 nets. **Proposed, locked, target, configured, implemented, modeled, measured, tested, certified, and production-proven are distinct statuses.** Implementation does not establish measured or certified performance.
...
|---|---|---|
| ADC pins and sequence | CH1 = PA0/ADC1_IN5, rank 1; CH2 = PA1/ADC1_IN6, rank 2 | Schematic implemented; firmware register/readback confirmation required |
| ADC clock/rate | ADC asynchronous clock target 80 MHz; timer-triggered 3.2 Msps aggregate; 12.5-cycle sample + 12.5-cycle conversion | Proposed configuration; ADC timing remains unmeasured |
...
| Analog architecture | Divider → CHx_CLAMP → 10 kΩ → TLV9062 non-inverting input → follower → 47 Ω/1 nF ADC RC | Implemented |
...
| Buffer/filter | TLV9062IDGKR; 1.0 nF C0G at ADC pin; R12/R18 = 47 Ω Yageo RC0603FR-0747RL | Implemented; bounded model passes, hardware timing/settling unmeasured |
...
| Backlight resistor | 27 Ω, 0.75 W initial prototype tuning value | Bounded value-selection gate: accept only if measured current is 60–75 mA |
| Backlight switching | Protected-5-V high-side PWM: NCE3401 P-MOS plus 2N7002KT1G gate pull-down; LCD pin 6 remains solid ground | Implemented; PWM high = backlight ON; operating point and thermals unmeasured |
...
`BNC center → 300 kΩ → 300 kΩ → 300 kΩ → divider node/CHx_CLAMP; 200 kΩ CHx_CLAMP-to-AGND; compensation capacitors across divider resistors; BAV199 clamps at CHx_CLAMP; 10 kΩ CHx_CLAMP-to-TLV9062 non-inverting input; TLV9062 follower; 47 Ω output series; 1 nF ADC-pin-to-AGND; MCU ADC pin.`
...
- The superseded 330 Ω/1 nF network modeled at approximately **9.5–9.9 LSB external acquisition error**. The implemented **47 Ω/1 nF** network models at **0.558 LSB worst case** over RADC bounds 0–680 Ω. This is not a measured ADC timing or settling result.
...
The backlight uses a protected-5-V high-side PWM switch. Protected VBUS connects to the **source of fitted NCE3401 P-channel MOSFET Q1**;

Logic polarity is non-inverting end-to-end: **MCU PWM high turns the 2N7002 on, pulls the NCE3401 gate low, turns the P-MOS on, and turns the backlight ON**. PWM low leaves both devices off through their pull resistors. The fitted NCE3401 has 75 mΩ maximum RDS(on) at VGS = −4.5 V;
...
The exact accounting is LDO input current = LDO output current + LDO quiescent current, with direct-VBUS loads added separately. Including 80 µA LDO IQ and 20 µA TVS leakage gives 125.10/145.10/175.10 mA; rounded totals are:
...
## Implemented PCB and release state

- Board: 100 × 100 mm, four copper layers, nominal 1.30 mm custom stackup; all components top-side.
- Routing: complete, zero airwires and zero DRC errors.
- USB_DP final repair: bottom-layer bypass with explicit top/bottom transition vias.
- USB configuration target: 45 Ω single-ended / 90 Ω differential; fabrication target only, not measured or certified performance.
- Remaining protected warnings: `Trace a4ae` and `Trace b74f`; Gerber cleanup or explicit waiver required.
- SMT assembly blocker: fourteen fabricated 1.0 × 1.0 mm test pads remain BOM elements and must be excluded from BOM and pick-and-place. TP_GND (Keystone 4952) remains included.
- DNI exclusion set: C16, C17, C23, and C24.
...
- Implemented items are not described as measured, tested, certified, or production-proven without evidence.

**Remaining schematic blocker:** none identified. Bandwidth, divider compensation, 15 V overload survival, backlight operating point, USB ESD/hot-plug, ADC timing, thermals, and HSI accuracy remain unmeasured prototype gates.